...
{  
    ...
    ConstructTubeMatrixDetector( "_1"
            , rot
    ...
    ConstructTubeMatrixDetector( "_2"
            , rot
            , G4ThreeVector(0.*cm, -caloGap_*cm, -caloParticleToCalorimeterDistance_*cm)
            , caloLayerThick_*cm
            , caloRMin_*cm
    ...
    // uncomment to see the world volume as a semi-transparent box
...
void DetectorConstruction::ConstructTubeMatrixDetector( const G4String& suffix
        , G4RotationMatrix * rot
    ...
    // Aggregating tube segment
    // (ctr fmt: name, innerR, outerR, half-Z, startingPhi, segmentAngle)
    G4VSolid * tube = new G4Tubs( "tube" + suffix
                                , rMin, rMax
                                , layerThick * layersNumberZ
                                , 0., phi);
    G4LogicalVolume * logTube = new G4LogicalVolume(tube,
            Mat("G4_Galactic"), "tubeL" + suffix, 0, 0, 0);
    new G4PVPlacement(rot, pos, "tubeP" + suffix, logTube, physWorld_, false, 0);

    // Layer within the aggregating tube segment along Z;
    // [optionally] includes R- and phi-segmentation
    G4VSolid * tubeZ = new G4Tubs("tubeZ" + suffix
                                 , rMin, rMax
                                 , layerThick
                                 , 0., phi);
    G4LogicalVolume * logTubeZ = new G4LogicalVolume(tubeZ,
            Mat("G4_Galactic"), "TubeZ" + suffix, 0, 0, 0);
    // (replcate the layer within aggregating tube)
    new G4PVReplica("TubeZ" + suffix
                   , logTubeZ, logTube
                   , kZAxis, layersNumberZ, layerThick*2);

    // Radial segmentation (within the longitudial)
    G4VSolid * tubeRadial = new G4Tubs( "tubeRadial" + suffix
                                      , 0, layerHeight
                                      , layerThick, 0., phi);
    ...
    // Azimuthal segmentation (within the radial)
    ...
    // Pb absorber layer (within elementary segment)
    ...
    // Plastic layer (within elementary segment)
...
void DetectorConstruction::ConstructCube( G4double distanceToCenter
                                        , G4double gap
                                        , G4int samplesNumber)
{
        ...
        const G4String& suffix)
{
    ...
    G4VSolid * mother = new G4Box( "mother" + suffix
                                 , sizeX/2, sizeY/2, pixelSize[2]/2 );
    G4LogicalVolume * logMother = new G4LogicalVolume( mother, Mat("G4_Galactic")
                                                     , "Mother" + suffix);
    ...
    G4VSolid * repX = new G4Box( "cubePixel"+suffix
                               , pixelSize[0]/2, pixelSize[1]/2,pixelSize[2]/2);
